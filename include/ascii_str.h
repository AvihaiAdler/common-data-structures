...
/**
 * @brief destroys an `ascii_str` object
 *
 * @param[in] ascii_str the string object to be destroyed
 */
 ...
 * @return `true` if the object is an empty string, `false` otherwise
 */
 ...
 *
 * @param[in] ascii_str an `ascii_str` object
 * @return `char const *` - a pointer to the underlying data of the string. the ptr behaves as a c-string and guarantee
...
/**
 * @brief erases `count` bytes (chars) from the string from position `pos` up to `pos + count - 1`
 * [`pos`, `min(count, ascii_str::len)`)
 *
 ...
 *
 * @param[in] ascii_str the string to split. this string may not change after this function call
